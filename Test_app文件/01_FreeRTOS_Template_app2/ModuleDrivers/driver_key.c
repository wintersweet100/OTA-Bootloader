...
*/
void LED_GPIO_ReInit(void)
{
	GPIO_InitTypeDef GPIO_InitStruct = {0};

	LED_GPIO_CLK_EN();//时钟使能

	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
	
	GPIO_InitStruct.Pin = LED1_RED;
	HAL_GPIO_Init(LED_PORT, &GPIO_InitStruct);	

	GPIO_InitStruct.Pin = LED2_GREEN;
	HAL_GPIO_Init(LED_PORT, &GPIO_InitStruct);	

	GPIO_InitStruct.Pin = LED3_BLUE;
	HAL_GPIO_Init(LED_PORT, &GPIO_InitStruct);	
	
	HAL_GPIO_WritePin(LED_PORT,LED1_RED,GPIO_PIN_SET);
	HAL_GPIO_WritePin(LED_PORT,LED2_GREEN,GPIO_PIN_SET);
	HAL_GPIO_WritePin(LED_PORT,LED3_BLUE,GPIO_PIN_SET);
}
/*
 *  函数名：void KEY_GPIO_ReInit(void)
 *  输入参数：无
...
/*
 *  函数名：void EXTI0_IRQHandler(void)/void EXTI15_10_IRQHandler(void)
 *  输入参数：无
 *  输出参数：无
 *  返回值：无
 *  函数作用：外部中断线0和10~15的中断服务函数
*/
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	if(KEY1_PIN == GPIO_Pin)    // 判断进来的外部中断线连接的引脚是不是按键的引脚
	{
		...
		HAL_GPIO_TogglePin(LED_PORT,LED2_GREEN);
		
	}
	else if(KEY2_PIN == GPIO_Pin)
	{
		key2_val = K2;
		HAL_GPIO_TogglePin(LED_PORT,LED3_BLUE);
	}
}
