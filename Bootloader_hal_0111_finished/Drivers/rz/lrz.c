...
extern void putdatas(const char *datas, int len);
...
{
	rz_t *rz = &g_rz; //(rz_t *)malloc(sizeof(rz_t));
...
size_t zmodem_receive(const char *directory,
		      bool approver_cb(const char *filename, size_t size, unsigned int date),
		      bool tick_cb(const char *fname, long bytes_sent, long bytes_total, long last_bps, int min_left, int sec_left),
		      void complete_cb(const char *filename, int result, size_t size, unsigned int date),
		      uint64_t min_bps,
		      uint32_t flags)
{
...
rz_process_header(rz_t *rz, char *name, struct zm_fileinfo *zi)
{
	...
	if (*nameend) {	/* file coming from Unix or DOS system */
		...
		long bytes_total;
		int mode;
		sscanf(nameend, "%ld%lo%o", &bytes_total, &modtime, &mode);//解析 ZMODEM 协议中跟随在文件名后的文件元数据（属性信息）
		zi->modtime=modtime;
	...
	if (rz->topipe > 0) {
...
#endif		
		if ((rz->fout=popen(name_static, "w")) == NULL)
			return ERROR;
	} else {
#if 0	
		if (rz->pathname)
		...
		rz->fout = fopen2(name_static, openmode);
		if ( !rz->fout)
		{
...
//				int secleft =  0;
				...
				last_bps= rz->min_bps; // zi->bytes_received/d;
				//if (last_bps > 0) {
