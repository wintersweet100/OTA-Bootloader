...
#if 0
/*定义默认IP信息*/
uint8 local_ip[4]  ={10,19,164,88};											/*定义W5500默认IP地址*/
uint8 subnet[4]    ={255,255,0,0};										/*定义W5500默认子网掩码*/
uint8 gateway[4]   ={10,19,0,1};											/*定义W5500默认网关*/
...
{	
	uint8_t version = IINCHIP_READ(0x0039); // 0x0039 是 W5500 的版本寄存器地址
   ...
   /*复制定义的配置信息到配置结构体*/
	...
	// 在 W5500 初始化或 Socket 打开后执行
	IINCHIP_WRITE(Sn_IMR(0), Sn_IR_RECV); // 开启 Socket 0 的中断屏蔽
	IINCHIP_WRITE(SIMR, 0x01);     // 全局开启 Socket 0 的中断触发
}
...
void gpio_for_w5500_config(void)
{
  ...
  /* SPI1 configuration */
  SPI_InitStructure.SPI_Direction = SPI_Direction_2Lines_FullDuplex;//全双工主机
